...
foreach (@list_url)#对我们自己合成的目标url做循环爬取适合的链接
		  ...
		  my $tree = HTML::TreeBuilder::XPath->new_from_content($html); # empty tree
          #$tree->parse($html) or print "error : parse html ";#$tree 为指向$html的hash变量
		  @dl_list=$tree->findnodes('//dl[@class="time_dl"]') or print "error : cannot find time_dl";
	foreach (@dl_list)
      {
