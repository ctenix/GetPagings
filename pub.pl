...
@list_url=();
@download_url=();
foreach (1..10)#首先自己看看需要爬多少个页面
        {
         my $url = URI->new('http://npd.nsfc.gov.cn/fundingProjectSearchAction!search.action');
         my($field,$page,$year,$size) = ("B06",$_,"2012","21");#提交表单参数，包括学科领域、年份以及每页显示条目数等
         $url->query_form
         (
           # All form pairs:
           'fundingProject.applyCode'  => $field, #领域代码B06表示化工，可替换成其它代码
           'currentPage' => $page,
		   'fundingProject.statYear' => $year,#年
		   'pageSize' => $size,#每页显示基金条目数
         );
         push @list_url,$url;
        }
...
foreach (@list_url)#对我们自己合成的目标url做循环爬取适合的链接
		  ...
		  @dl_list=$tree->look_down(_tag,'dl') or print "error : cannot find time_dl";#成功找到dl,dl标签包含基金项目信息
		  
	foreach (@dl_list)#遍历每个网页中的dl标签，并输出文字信息
      {
	  print FH $_->as_text()."\n";#
      }
}
